...
description <- paste0(policy[, "Guiding Policy"], ": ", 
                     policy[, "Description"])
resource <- policy["URL"]
csa <- policy[, "CSA Goals"]

practice <- unique(hicsa.db[, "Practice"])
practice
...
## Create repeated entries matching policy to CSA practice
## Then bind to hicsa.db
...
grants <- read_sheet("https://docs.google.com/spreadsheets/d/1ljbwFQuHo0Xn0u0ZD60YKZk8SdglJl5tpOgjSWbY9gA/",
                     sheet = 1)
grants <- as.data.frame(grants[, c("Mitigation", "Practice", "Category", "Resources (Links)")])
...
```{r db-merge, eval = FALSE, echo = TRUE}
